...
版本：v1.1  
目标：形成一套标准、严格、高性能、可模块化实现的算法程序架构，支持多机位、多光源频闪图像并行处理。  
...
class FeatureBuilder:
    ...
    def build(self, reflectance_cubes, recipe):
        """为每个机位、每个 ROI 构建模型输入特征。"""
        feature_groups = []
        for cube in reflectance_cubes:
            feature_groups.append(self._build_roi_features(cube, recipe))
        return feature_groups

    def _build_roi_features(self, cube, recipe):
        """构建单个 ROI 的多通道特征。"""
        diffuse = cube.get("DIFFUSE")
        polar = cube.get("POLAR_DIFFUSE")
        high_left = cube.get("HIGH_LEFT")
        high_right = cube.get("HIGH_RIGHT")
        high_front = cube.get("HIGH_FRONT")
        high_rear = cube.get("HIGH_REAR")
        low_left = cube.get("LOW_LEFT")
        low_right = cube.get("LOW_RIGHT")
        low_front = cube.get("LOW_FRONT")
        low_rear = cube.get("LOW_REAR")

        features = [
            self._required(diffuse, "raw_diffuse"),
            self._optional(polar, "raw_polar"),
            self._abs_diff(high_left, high_right, "high_lr_diff"),
            self._abs_diff(high_front, high_rear, "high_fr_diff"),
            self._max_min([high_left, high_right, high_front, high_rear], "high_max_min"),
            self._abs_diff(low_left, low_right, "low_lr_diff"),
            self._abs_diff(low_front, low_rear, "low_fr_diff"),
            self._max_min([low_left, low_right, low_front, low_rear], "low_max_min"),
            self._sobel(diffuse, "sobel_diffuse"),
            self._laplacian(diffuse, "laplacian_diffuse"),
            self._local_contrast(diffuse, "local_contrast"),
            self._specular_removed(diffuse, polar, "specular_removed"),
        ]
    ...
    def _stack(self, features):
        """将多个二维特征堆叠为 C x H x W 的张量。"""
        pass
```
...
4. 若关键光源缺失，按配方进入 `RECHECK` 或降级；禁止静默补图后输出 OK。
5. 若 `RegistrationReport.is_pass == false`，该 ROI 不允许输出 OK。
6. 若使用低角度暗场图构建划伤特征，至少需要成对方向光，例如 `LOW_LEFT + LOW_RIGHT` 或 `LOW_FRONT + LOW_REAR`。
7. 若使用高角度差分构建压痕/凹坑特征，至少需要成对方向光，例如 `HIGH_LEFT + HIGH_RIGHT` 或 `HIGH_FRONT + HIGH_REAR`。
...
cameras:
  TOP_BACK:
    enabled: true
    model_key: top_back_black_leather
...
建议统一以下光源 ID，未启用的通道由配方控制：

| light_id | 用途 |
|---|---|
| DIFFUSE | 漫射光，检测颜色、脏污、整体纹理 |
| POLAR_DIFFUSE | 交叉偏振漫射，抑制高光 |
| HIGH_LEFT | 左高角度光，增强凹凸方向性 |
| HIGH_RIGHT | 右高角度光，增强凹凸方向性 |
| HIGH_FRONT | 前高角度光，增强纵向起伏 |
| HIGH_REAR | 后高角度光，增强纵向起伏 |
| LOW_LEFT | 左低角度暗场，检测细划伤和凸起 |
| LOW_RIGHT | 右低角度暗场，检测细划伤和凸起 |
| LOW_FRONT | 前低角度暗场，检测前后向细划伤 |
| LOW_REAR | 后低角度暗场，检测前后向细划伤 |
| NIR | 近红外，可选，用于深色/织物材料 |

### 10.2 标准特征集合

| 特征 | 来源 | 作用 |
|---|---|---|
| raw_diffuse | DIFFUSE | 颜色、脏污、整体纹理 |
| raw_polar | POLAR_DIFFUSE | 去高光后的真实纹理 |
| high_lr_diff | HIGH_LEFT - HIGH_RIGHT | 左右方向压痕、凹坑 |
| high_fr_diff | HIGH_FRONT - HIGH_REAR | 前后方向压痕、起皱 |
| high_max_min | 高角度 max-min | 表面起伏响应 |
| low_lr_diff | LOW_LEFT - LOW_RIGHT | 细划伤、凸起边缘 |
| low_max_min | 低角度 max-min | 暗场异常增强 |
| sobel_diffuse | DIFFUSE | 常规边缘和纹理 |
| local_contrast | DIFFUSE / POLAR | 低对比缺陷 |
| specular_removed | DIFFUSE + POLAR | 高光抑制后的表面信息 |
...
| 同机位 ROI 标准化 | 将曲面区域裁剪/展开为模型输入 ROI | 必须 |
...
2. 默认对齐方法为 `fixed_calibration`，即使用离线标定矩阵。
3. 不建议在线每张图做特征点匹配，因为不同光源下纹理和高光差异大，容易不稳定。
4. 若座椅姿态变化导致固定矩阵不够，应先做整椅关键点/ROI 定位，再对 ROI 做局部仿射或单应矫正。
...
4. 划伤、压痕、凹坑、起皱应充分利用低角度暗场和多方向差分特征。
5. 脏污、色差、油污应充分利用漫射光、交叉偏振和颜色稳定性。
6. 已知缺陷优先使用监督分割或检测模型；未知缺陷使用异常检测兜底。
