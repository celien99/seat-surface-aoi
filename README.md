...
**生产配方默认启用空间 PatchCore 模式（`spatial_mode: true`）**，检测链路提取 layer2+layer3 中间层特征图构建 H×W patch 嵌入，当前原始 patch embedding 为 1536 维，经 `seat_pca.json`（v2, 524 维，95% 累积方差）投影后进入 PatchCore memory bank/FAISS。批量 PCA 会把 patch embedding 显式转换为二维 `numpy` 矩阵，并通过 `size/ndim/shape` 校验空输入和维度，不再对数组做布尔判断。KNN 评分后生成像素级 `anomaly_map`，经 `scipy.ndimage.label` 连通域分析自动定位缺陷区域并输出 bbox。在线推理会先把 `anomaly_map` 和 `nearest_distances` 归一化为二维有限矩阵并确认形状一致，再做最大值统计和连通域提取，避免 numpy 数组布尔歧义或异常形状静默进入判定。TraceWriter 同时写两类图：`patchcore_heatmaps/` 保存原生连续 anomaly_map 伪彩调试图；`overlays/` 只在最终缺陷候选 bbox 内叠加达到 PatchCore 二值化阈值的黄/红热色，其他低分 ROI 保持灰度原图，并绘制判定框和缺陷 bbox，供前端生产展示通道消费。若需回退到全局嵌入路径（整个 ROI → 1 个标量分数），在配方中设置 `spatial_mode: false`。
