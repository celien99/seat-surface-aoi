...
- 相机链路：本地回归 `simulated`，现场 `hikrobot_mvs`；真实采集对齐现场可工作的参考程序，先排空相机 SDK 缓存并 arm 相机，再由 FL-ACDH 触发曝光，最后调用 `GetImageBuffer` 读取已缓存的硬触发帧。当前现场配置使用 10ms 相机曝光和 100/200/300us 三路频闪脉宽，避免 FL-ACDH 拒绝 650-700us 脉宽命令。单台相机连续失败后自动 stop+start 重启恢复。
