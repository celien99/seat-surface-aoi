...
uv export --format requirements.txt --frozen --no-hashes --no-emit-project --no-dev `
  --extra onnx --extra faiss --extra display --extra opencv `
  --output-file wheelhouse\seat-aoi-runtime-requirements.txt
...
- 同步 `model/` 下已存在的真实资产到当前 `ModelRoot`；源文件是占位文件时跳过，目标已有同名文件时直接覆盖
...
- `python -m tools.validate_model_assets --recipe seat_a_black_leather_production_v1`，同时校验 PatchCore memory bank 的 `distance_mean`/`distance_p99` 自校准统计量和 `thresholds` 判定阈值产物；安装脚本只负责把当前 `model/` 下存在的同名资产复制到 `ModelRoot`，若项目 `model/` 没有可同步的新资产，才需要运行 `training_tools.build_patchcore_memory_bank calibrate` 或手动替换目标模型文件后再启动服务。
...
**生产配方默认启用空间 PatchCore 模式（`spatial_mode: true`）**，检测链路提取 layer2+layer3 中间层特征图构建 H×W patch 嵌入，当前生产配方显式使用 `128x128` 空间 patch 网格，原始 patch embedding 为 1536 维，经 `seat_pca.json`（v3, 524 维，95% 累积方差）投影后进入 PatchCore memory bank/FAISS。离线训练和最终 memory bank 向量均使用 `.npy` 二进制矩阵保存，`seat_patchcore_bank.json` 保存版本、维度、`vectors_path`、向量数、metadata、校准统计和训练阈值。**KNN 评分采用训练数据 held-out 自校准**：`anomaly_score = tanh((nearest - μ) / (2 * max(p99 - μ, ε)))`，μ 和 p99 由训练阶段对 memory bank 做 held-out 校准自动计算并写入 `bank.json` 的 `distance_mean`/`distance_p99` 字段；同一校准阶段还会基于正常样本 bootstrap 最大异常分布写入 `thresholds.recheck_score` 和 `thresholds.ng_score`。在线 PatchCore 主模型只使用 bank thresholds 生成候选并判 `RECHECK/NG`，生产 YAML 不再保留 `score_threshold`、`decision_threshold.ng_score` 或 `decision_threshold.recheck_score` 作为 PatchCore 判定阈值；即使外部配方带入旧 `score_threshold` 字段，PatchCore 加载时也会忽略。`tools.validate_model_assets` 会把校准统计或 thresholds 缺失作为阻塞错误。本次仅对现有 `seat_patchcore_bank.npy`/FAISS 做阈值自校准，没有重训模型。KNN 评分后生成像素级 `anomaly_map`，经 `scipy.ndimage.label` 连通域分析自动定位缺陷区域并输出 bbox。在线推理会先把 `anomaly_map` 和 `nearest_distances` 归一化为二维有限矩阵并确认形状一致，再做最大值统计和连通域提取，避免 numpy 数组布尔歧义或异常形状静默进入判定。TraceWriter 热力图渲染管线使用**双线性插值**上采样 anomaly_map → 高斯平滑消除栅格残余 → 形态学闭运算填补掩码孔洞，最终仅在缺陷候选 bbox 内叠加黄/红热色，并绘制判定框和缺陷 bbox 供前端生产展示通道消费。若需回退到全局嵌入路径（整个 ROI → 1 个标量分数），在配方中设置 `spatial_mode: false`。
