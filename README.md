...
- `python -m tools.validate_model_assets --recipe seat_a_black_leather_production_v1`，同时校验 PatchCore memory bank 的 `distance_mean`/`distance_p99` 自校准统计量；安装脚本只负责把当前 `model/` 下存在的同名资产复制到 `ModelRoot`，若项目 `model/` 没有可同步的新资产，才需要运行 `training_tools.build_patchcore_memory_bank calibrate` 或手动替换目标模型文件后再启动服务。
