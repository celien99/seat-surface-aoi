...
- Python 质量门禁会校验必需光源的采集一致性，包括时间戳跨度、时间戳单调性、帧序号重复、光源序号重复、`light_seq_index` 与配方光源顺序一致性、曝光差和增益差；会在预处理前拒绝非 `MONO8/UINT8/MONO/1ch`、stride 小于有效行宽或图像长度不足的帧；灰度、饱和、清晰度和运动模糊统计只使用有效像素宽度，不把 stride padding 当成图像内容；必需光源亮度漂移、运动模糊或异常采集包会返回 `RECHECK`。
...
- 配方已覆盖机位、光源顺序、质量阈值、注册策略、ROI 级主模型、unknown safety net 模型、模型后端和追溯配置；schema 会拒绝越界质量阈值、越界分数阈值、负面积阈值、`recheck_score > ng_score` 以及模型 `class_names` 缺少显式 `thresholds` 的不安全规则配置。
- 配方加载阶段会校验全局/机位级 `light_order`、`quality.required_lights`、`registration.base_light_id` 和 `registration.base_light_fallback` 的一致性；基准光源、fallback 光源或必需光源未启用时会提前拒绝配方，避免运行中退化到不可追溯的注册失败。
