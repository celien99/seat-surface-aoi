...
| 共享内存 IPC | C++/Python 双端固定布局结构体、frame/result ring buffer、slot 状态机、CRC、layout fail-fast 和协议校验工具。 |
| V4 算法接口 | Dome ROI YOLO、ECC 配准、WideResNet50 embedding、PCA、PatchCore KNN 和 FAISS 可选加速接入点。 |
| 保守判定 | 协议异常、CRC 错误、缺帧、超时、质量失败、机器人 FAULT、候选融合溢出和模型异常返回 `RECHECK` 或 `ERROR`。 |
...
协议当前为 `SHM_PROTOCOL_VERSION = 2`，固定小端布局，包含 slot 状态机、header CRC 和 payload CRC。C++ 打开既有共享内存时会校验 magic/version/slot_count/slot_size，不匹配且未显式 reset 时直接失败，避免静默重写布局；结果 ring 会回收旧序号或坏状态 slot，但当前序号的 corrupted/timeout 仍按 CRC 或 detector timeout 保守失败。协议细节见 [共享内存协议](docs/shm_protocol.md)。
