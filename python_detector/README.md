...
`RecipeManager` 默认从包内 `python_detector/config` 加载 YAML，不依赖当前工作目录；可通过 `--recipe-dir` CLI 参数显式指定配方目录。在线 `DetectorProcess` 收到 `--recipe-dir` 后会把同一目录同时传给 `RecipeManager` 和 `Preprocessor` 内部的 `CalibrationManager`，确保打包后的服务读取安装目录下的配方、标定和 ROI 模板，而不是 PyInstaller onefile 的临时 `_MEI...` 解包目录。`CalibrationManager` 通过 `paths.resolve_package_path()` 同时兼容包内路径和历史仓库相对路径，例如 `python_detector/config/roi/default_roi.yaml`。PyInstaller 打包后（`sys.frozen=True`），`paths.py` 自动从 `sys.executable` 推导项目根目录和配置目录，不再依赖 `__file__`；路径解析优先使用安装目录 `python_detector/config`，exe 内置的 `python_detector/config` 仅作为外部配置缺失时的兜底资源。
