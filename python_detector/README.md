...
uv run python -m python_detector.detector_main `
  ...
  --recipe-dir python_detector/config `
  --once --timeout-ms 8000
```

`tools.validate_model_assets` 会校验 ONNX/PCA/bank/FAISS 是否存在且不是占位文件，并检查当前 PatchCore 链路的维度一致性：配方 `embedding_dim`、PCA 输入维度、PCA 输出维度、memory bank 维度、FAISS 维度/向量数以及 bank `distance_mean`/`distance_p99` 自校准统计量和 `thresholds` 判定阈值产物必须对齐和齐备。

Windows `install_station.ps1` 在注入 active recipe 到 `ModelRoot` 后，只负责把项目根目录 `model/` 下已存在的真实模型资产复制到当前 `ModelRoot`。源文件是占位文件时跳过；目标已有同名资产时直接覆盖，避免 `D:\seat-aoi-model` 保留缺少 `distance_mean/distance_p99` 或 `thresholds` 的旧 PatchCore bank 后导致安装期模型验证失败。
...
├── ipc/
│   ├── data_types.py           # LightFrame、CameraBundle、SeatInspectionJob、InspectionResult 等数据结构；SeatInspectionJob 保留 capture_mode 元数据
...
- `DetectorProcess`：在线进程入口。只负责初始化 `ShmClient`、等待任务、调用算法 facade、先写最小 `result.json`、发布结果、后台补齐 raw/overlay trace，并释放共享内存 slot。
...
事件字段包含 `sequence_id`、`trigger_id`、`seat_id`、`sku`、`recipe_id`、`decision`、`quality_pass`、`error_code`、`elapsed_ms`、缺陷列表、质量/错误消息、`sample_collection`、`trace_dir`、原始采集 PNG 图、ROI PNG 图和检测 overlay PNG 图路径。展示通道由本仓库 `display_app/` 的 PySide6/QML 前端只读消费，也可供外部 `online-detection-app` 对接；它不读写现有 C++/Python 共享内存 slot。在线链路在写回共享内存前只同步写 `result.json`，该最小结果写入失败会把当前件改为 `RECHECK/DEVICE_FAULT`，避免无法追溯时继续输出 `OK`；raw PNG 和 overlay 在结果发布后后台补齐，失败只打印告警，不反向修改已经发布的共享内存结果。图片补齐后 `display_latest.json` 会刷新同一 `sequence_id/trigger_id/timestamp_ms` 事件，前端只刷新图片，不重复统计或重复弹 NG 弹窗。采集失败、detector timeout 等 C++ 侧保守结果可由前端读取 `trace_root/cpp_controller_events.jsonl` 补充显示。
...
`RecipeManager` 默认从包内 `python_detector/config` 加载 YAML，不依赖当前工作目录；可通过 `--recipe-dir` CLI 参数显式指定配方目录。在线 `DetectorProcess` 收到 `--recipe-dir` 后会把同一目录同时传给 `RecipeManager` 和 `Preprocessor` 内部的 `CalibrationManager`，此时标定和 ROI 必须存在于该外部目录，缺失会直接失败，不再回退到 PyInstaller onefile 的临时 `_MEI...` 解包目录。`CalibrationManager` 通过 `paths.resolve_package_path()` 同时兼容包内路径和历史仓库相对路径，例如 `python_detector/config/roi/default_roi.yaml`。PyInstaller 打包后（`sys.frozen=True`），`paths.py` 自动从 `sys.executable` 推导项目根目录和配置目录，不再依赖 `__file__`；路径解析优先使用安装目录 `python_detector/config`，exe 内置的 `python_detector/config` 仅作为未显式传入外部配方目录时的兜底资源。模型字段中的 `model/...` 相对路径由 `paths.resolve_runtime_path()` 解析到部署模型根或项目 `model/`，不依赖当前工作目录；安装脚本仍会把 active recipe 注入为当前 `ModelRoot` 绝对路径并做 fail-fast 校验。`--config` 相对路径找不到时会按部署根兜底解析，运行配置里的 `trace_root` 会覆盖算法 trace 写入根目录，避免 display JSON 和 raw/overlay trace 分裂。`--validate-config-only` 只加载运行配置、所有配方、标定和 ROI 模板后退出，不创建共享内存、不控制 PLC/相机/频闪，安装脚本用它验证打包 exe 的配置路径。
...
- 模型输入通道、监督/模拟模型的 `decision_threshold`、bbox 格式和输出 decode 规则；PatchCore 生产主模型的判定阈值来自 memory bank `thresholds` 训练产物。
...
- `fake`：测试和模拟链路默认后端。
- `onnx`：可选 ONNX detection rows 后端，要求 `onnxruntime` 和 `numpy`；ONNX Runtime session 启用图优化，YOLO detection/segmentation 输出解码使用数组化 class argmax、score filter、bbox 转换和分块 mask logits 计算，并在保留候选前拒绝非有限分数，保持 Python float 阈值边界语义。
- `patchcore_knn`：PatchCore 无监督异常检测主模型或可选安全网，使用 statistical 或 ONNX embedding、可选 PCA、memory bank；PCA 批量投影使用 `float32`，配置 `faiss_index_path` 时优先尝试 FAISS，失败时回退 exact KNN，并在 `anomaly_summary` 写入实际 backend 和 fallback reason。
...
空间模式要求：`embedding_backend=onnx_wideresnet50`、`spatial_layers` 非空（如 `[layer2, layer3]`），并使用 `--spatial-mode` 重新导出 ONNX 模型和重新训练记忆库。**生产配方默认启用 `spatial_mode: true`**，当前生产配方显式使用 `128×128` 空间 patch 网格，layer2+layer3 原始 patch embedding 为 1536 维，在线先用 `seat_pca.json`（v3, 524维, 95%累积方差）投影，再进入 PatchCore bank/FAISS 评分并生成像素级 anomaly_map 热力图；热力图渲染使用双线性插值+高斯平滑，`128×128` 网格可平滑映射到 ROI 原图分辨率，无马赛克效应。若需回退到全局嵌入路径（整个 ROI → 1 个向量 → 标量分数），在配方中设置 `spatial_mode: false`。anomaly_map 二值化阈值可通过 `anomaly_binarize_min_ratio`（默认 0.5）和 `anomaly_binarize_relative`（默认 0.3）配置，阈值公式为 `max(bank.thresholds.recheck_score × min_ratio, max_anomaly × relative)`。

**异常分数自校准和训练阈值**：KNN 距离到异常分数采用训练数据 held-out 分位数归一化。训练阶段将 bank 向量分半：一半做缩减 bank，一半做 held-out 查询，自动计算正常样本的距离均值 `distance_mean` 和 p99 代表距离 `distance_p99`，写入 `seat_patchcore_bank.json`。推理时评分公式为 `anomaly_score = tanh((nearest - distance_mean) / (2 * max(distance_p99 - distance_mean, ε)))`。校准工具还会对正常样本 bootstrap 最大异常分布取分位数，写入 `thresholds.recheck_score` 和 `thresholds.ng_score`；在线 PatchCore 主模型只使用 bank thresholds 判定候选和 `RECHECK/NG`，生产 YAML 不再保留手工 `score_threshold/ng_score/recheck_score` 作为 PatchCore 分数阈值，外部配方带入旧 `score_threshold` 时也会被 PatchCore 加载逻辑忽略。校准统计量和 thresholds 可在训练完成后单独补充：`python -m training_tools.build_patchcore_memory_bank calibrate --vectors ... --bank ... --faiss-index ... --recheck-quantile 0.99 --ng-quantile 0.999`；本次只对现有 bank 向量和 FAISS 索引重新校准阈值，没有重训模型。`tools.validate_model_assets` 会在安装和预检阶段拦截缺少这些字段的旧 bank。
...
`FusionEngine` 对同一 camera/pose/ROI 执行 IoU NMS，合并 evidence lights，并限制每个 ROI 候选数。NMS 抑制数和候选容量溢出数分开统计；如果容量溢出隐藏了候选且规则原本会输出 `OK`，`RuleEngine` 会改为 `RECHECK` 并写入 `CONFIGURATION_ERROR`，避免用 `OK` 掩盖算法不确定性。`DefectFilter` 和 `RuleEngine` 对 PatchCore 候选优先使用候选携带的 bank thresholds 判定 `NG/RECHECK`，对 fake/ONNX 等非 PatchCore 候选继续使用配方 `decision_threshold`；生产 PatchCore 配方的 `decision_threshold` 只保留面积和长宽比过滤参数。长宽比过滤（`min_aspect_ratio` / `max_aspect_ratio`）用于排除长条形噪声，不通过的 NG 候选降级为 RECHECK。缺陷结果不携带缺陷类别字段，只表达位置、分数、证据光源和处置判定。

`TraceWriter` 按配方 trace 策略保存。在线 `DetectorProcess` 使用轻量路径：发布共享内存结果前仅创建 trace 目录并写入扁平 `result.json`，随后后台补齐 raw PNG 和 overlay PNG；离线/API 调用 `TraceWriter.write()` 仍会写完整诊断 JSON：
...
- `timings.json`
- `error.json`
- 原始采集 PNG 图、ROI PNG 图和检测 overlay PNG 图；当前在线补图写入扁平路径：`raw_images/<camera_id>[_<pose_id>]_<light_id>.png`、`overlays/<camera_id>[_<pose_id>]_<roi_name>.png`。overlay 以匹配光源的 raw 原图为底图，尺寸等于 raw 原图；anomaly_map 从 ROI 坐标映射回 raw 坐标后只在最终缺陷候选 bbox 内叠加达到二值化阈值的热区，低分区域不再整片染蓝或染黄，缺陷 bbox 按 raw 坐标绘制。PNG filter scanline 构造、raw 灰度转 RGB、heatmap 上采样/坐标映射/alpha blend 和矩形绘制均使用 `numpy` 批量处理，仍使用内置 zlib PNG writer，不新增图像编码依赖。只要该 ROI 已完成预处理，OK、NG、RECHECK 和 ERROR trace 都会写检测 overlay；NG/RECHECK/ERROR 有缺陷候选时额外绘制候选 bbox。

在线 `DetectorProcess` 写 `result.json` 失败时，会记录 `context["trace_error"]`，并把当前结果改为 `RECHECK`、`error_code=DEVICE_FAULT`、`quality_pass=false` 后再交给 `ShmClient` 写回 C++。后台 raw/overlay trace 和展示通道 `display_latest.json` / `display_events.jsonl` 属于结果发布后的只读前端辅助输出，失败只打印告警，不反向修改已经发布的共享内存结果。
...
uv run python -m training_tools.train_patchcore_assets --manifest datasets/seat_roi_train/dataset_manifest.jsonl --output-dir model/patchcore --recipe seat_a_black_leather_production_v1 --model-key patchcore_detector --embedding-backend onnx_wideresnet50 --embedding-model model/wideresnet50/seat_wrn50_embedding.onnx --spatial-mode --spatial-layers layer2,layer3 --spatial-upsample-height 128 --spatial-upsample-width 128 --pca-components 524 --pca-version pca_seat_v3 --bank-version bank_v5_spatial128 --coreset-ratio 1.0 --coreset-method stride --build-faiss
