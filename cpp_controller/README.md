...
上述 `codex-check` 构建只适合在开发机做无硬件逻辑和 IPC 回归；没有 Hikrobot MVS SDK、相机、频闪和外部信号端的环境，不能作为生产硬件版本验收依据。
...
```powershell
# 1. Python detector，必须使用同一份 production.conf 和安装目录配方
...
while ($true) {
    $proc = Start-Process -FilePath ".\.venv\Scripts\python.exe" `
        -ArgumentList "-m","python_detector.detector_main","--config","cpp_controller\config\station_runtime.production.conf","--recipe-dir",".\python_detector\config" `
