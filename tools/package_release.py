...
tools/               # 协议、模型资产、架构检查、模拟 IPC 和 Windows 交付安装脚本
docs/                # 架构、共享内存协议和运维文档
```
...
```powershell
# 生产交付：管理员 PowerShell，注册 detector/controller 后台服务并创建 display_app 桌面快捷方式。
# nssm.exe 需位于 bin/nssm.exe、tools/nssm/nssm.exe 或 PATH。
powershell -ExecutionPolicy Bypass -File .\tools\windows\install_station.ps1 `
  -LineId LINE1_AOI_01 `
  -GridLayout 2x1

# 如果需要在工控机本机构建 Hikrobot MVS 版 C++ 主控，追加：
powershell -ExecutionPolicy Bypass -File .\tools\windows\install_station.ps1 `
  -BuildController `
  -EnableHikrobotMvs `
  -LineId LINE1_AOI_01 `
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        _copy_tree(ROOT_DIR / "tools" / "windows", tools_dir / "windows")
