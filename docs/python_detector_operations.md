...
缺 Dome 图、输出越界、置信度不足、seg mask 面积异常、越出安全边界、姿态误差超差或类别未映射时返回 `RECHECK`。
配置了 `input_width/input_height` 时，ROI 定位输入会先 letterbox 到模型训练尺寸；`input_channels=3` 会把 Mono8 Dome 图复制为 3 通道，匹配 Ultralytics segmentation ONNX 的常见输入。
