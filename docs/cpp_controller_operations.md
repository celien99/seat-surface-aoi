...
多控制器频闪使用控制器级 `light.<M>.<field>` 与通道级 `light.<M>.<N>.<field>`。当前产线只允许 1 台 FL-ACDH，下例仅作为未来扩展示例，不能直接用于当前生产配置：

```ini
light.0.backend=serial_ascii
light.0.serial_port=COM3
light.0.baud_rate=9600
light.0.response_mode=ack
light.0.trigger_input_line=F1
light.0.1.physical_channel=1

light.1.backend=serial_ascii
light.1.serial_port=COM4
light.1.baud_rate=9600
light.1.response_mode=ack
light.1.trigger_input_line=F1
