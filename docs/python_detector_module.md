...
- `seat-aoi-detector`：安装包后提供的命令行入口，等价于 `uv run python -m python_detector.detector_main`。
...
项目根目录 `pyproject.toml` 和 `uv.lock` 是 Python 算法层的依赖、虚拟环境和锁定版本入口：

- 默认依赖：`PyYAML`，用于配方、标定和 ROI YAML。
- `test` dependency group：`pytest`、`numpy`，用于算法层单元测试和 ONNX 输出解析测试。
- `dev` dependency group：`pytest`、`numpy`、`ruff`，用于本地开发验证。
- `onnx` extra：`numpy`、`onnxruntime`，仅在启用 ONNX/YOLO/WideResNet50 后端时需要。
...
- 新增配置字段必须同步 schema、默认配方、测试、README 和 `python_detector/README.md`。
